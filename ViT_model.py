...
class Attention(nn.Module):
    def __init__(self, dim, num_patches, heads = 8, dim_head = 64, dropout = 0.):
        ...
        self.attn_matrix = nn.Parameter(torch.empty(heads, (num_patches + 1, num_patches + 1)))
        self.attn_matrix.requires_grad = False

    def forward(self, x, mode, set_weights):
        ...
        if mode == 'standard':
            ...
            if set_weights:
                self.attn_matrix.data = attn.mean(dim = 0)
        if mode == 'test_weight_matrix':
...
class Transformer(nn.Module):
    ...
    def forward(self, x, mode, set_weights):
        for attn, ff in self.layers:
            x = attn(x, mode = mode, set_weights = set_weights) + x
            x = ff(x) + x
        return x
...
class ViT(nn.Module):
    ...
    def forward(self, img, mode = 'standard', set_weights = False):
        ...
        x = self.dropout(x)
        x = self.transformer(x, mode = mode, set_weights = set_weights)
